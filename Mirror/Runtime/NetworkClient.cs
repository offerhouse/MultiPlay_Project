...
namespace Mirror
{
    ...
    {
        public delegate void Local_Tower_Active_From_Pool();
        public static event Local_Tower_Active_From_Pool local_tower_active_from_pool;
        ...
        static NetworkIdentity SpawnPrefab(SpawnMessage message)
        {
            if (GetPrefab(message.assetId, out GameObject prefab))
            {
                ...
                if (pool_Manager)
                {
                    obj = pool_Manager.GetComponent<Pool_Manager>().Get_Object_From_Pool_By_Tag(prefab.tag);
                    if (obj)
                    {
                        Debug.Log("Get_Pool || " + obj + " || " + prefab.tag);
                        obj.SetActive(true);
                        if (prefab.tag == "Tower")
                        {
                            //obj.GetComponent<Tower>();
                            local_tower_active_from_pool?.Invoke(); // go here > Tower.Local_Tower_Set_Visible();
                        }
                    }
                }
                if (!obj)
                {
                    ...
                    Debug.Log("Instantiate || " + pool_Manager + " || " + prefab.tag);
                    List<GameObject> List = pool_Manager.GetComponent<Pool_Manager>().Get_Pool_List(prefab.tag);
                    if (List != null)
        ...
        public static void PrepareToSpawnSceneObjects()
        {
            ...
            foreach (NetworkIdentity identity in allIdentities)
            {
                ...
                {
                    Debug.Log("identity || " + identity.gameObject.name);
        ...
        internal static void OnObjectSpawnFinished(ObjectSpawnFinishedMessage _)
        {
            ...
            if (!Local_ProFile)
            {
                Debug.LogWarning("Local_ProFile || " + Local_ProFile);
                return;
            }

            Local_ProFile.GetComponent<Mirror_Info>().Spawn_Scene_Object_Finish = true;
            Debug.Log("Spawn_Scene_Object_Finish || " + Local_ProFile.GetComponent<Mirror_Info>().Spawn_Scene_Object_Finish);
        }
...
#pragma warning restore 618
        ...
        {
            Debug.Log("DestroyAllClientObjects");
