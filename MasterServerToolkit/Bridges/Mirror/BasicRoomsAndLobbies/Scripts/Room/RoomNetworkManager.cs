...
namespace MasterServerToolkit.Bridges.MirrorNetworking
{
    public class RoomNetworkManager : NetworkManager
    {
        ...
        public RoomPlayer Get_Profile_By_Room_Connection(int connectionID)
        {
            ...
            if (Room_Server != null)
            {
                RoomPlayer room_player = Room_Server.GetComponent<RoomServerManager>().GetRoomPlayerByRoomPeer(connectionID);
                if (room_player != null)
                {
                    Debug.Log("return room_player");
                    return room_player;
                }
            }
            return null;
        ...
        public void Set_User_ID_To_Player_Obj(int connectionID, GameObject player_Obj)
        {
            GameObject Room_Server = GameObject.Find("-- MIRROR_ROOM_SERVER");

            if (Room_Server != null)
            {
                RoomPlayer profile = Room_Server.GetComponent<RoomServerManager>().GetRoomPlayerByRoomPeer(connectionID);

                if (profile == null)
                    MstTimer.WaitForSeconds(2f, () =>
                    {
                        Set_User_ID_To_Player_Obj(connectionID, player_Obj);
                    });

                if (profile != null)
                {
                    string User_ID = profile.UserId;
                    player_Obj.GetComponent<NetworkIdentity>().User_ID = User_ID;
                }
            }
        }
    }
